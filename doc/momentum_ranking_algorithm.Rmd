...
daysToSeconds <- function(seconds) { seconds * 60 * 60 * 24 }
secondsToDays <- function(seconds) { seconds / 60 / 60 / 24 }

colorLikes = "lightblue"
colorComments = "lightpink"

likeMomentumDecayFactor <- .00001
likeWeight <- 1

commentMomentumDecayFactor <- .00001
commentWeight <- 2.5

maxThresholdDays <- 30
thresholdDecayPeriodDays <- 730
```
...
eventAge <- seq.int(0, 10 * 24 * 60 * 60)
momentum <- likeWeight * ((1 - likeMomentumDecayFactor) ^ eventAge)
df <- data.frame(secondsToDays(eventAge), momentum)
plot(df,
    xlab = "Event age (days)",
    ylab = "Momentum",
    type = 'l',
    ylim = c(0, max(momentum))
)
```

Example result where $l_w = `r likeWeight`$ and $l_f = `r format(likeMomentumDecayFactor, scientific = FALSE)`$, considering all calculations made in seconds
...
timeFrame <- 10 * 24 * 60 * 60
eventAge <- seq.int(0, timeFrame)
...
momentumAt <- function(eventAge) {
    likeWeight * ((1 - likeMomentumDecayFactor) ^ eventAge)
}

barplot(
    momentumAt(likeEvents),
    names.arg = secondsToDays(likeEvents),
    xlab = "Days ago",
    ylab = "Contributed momentum",
    col = c(colorLikes)
)
```

Example showing how much 10 events contribute to the instant momentum according to their age, where $l_w = `r likeWeight`$ and $l_f = `r format(likeMomentumDecayFactor, scientific = FALSE)`$, considering all calculations made in seconds at column zero. Each column represents a single _like_ event.
...
likesMomentumAt <- function(eventAge) {
    ifelse(eventAge %in% likeEvents,
        likeWeight * ((1 - likeMomentumDecayFactor) ^ eventAge),
        0
    )
}

commentsMomentumAt <- function(eventAge) {
    ifelse(eventAge %in% commentEvents,
        commentWeight * ((1 - commentMomentumDecayFactor) ^ eventAge),
        0
    )
}

barplot(
    rbind(likesMomentumAt(instants), commentsMomentumAt(instants)),
    beside = TRUE,
    names.arg = secondsToDays(instants),
    xlab = "Days ago",
    ylab = "Contributed momentum",
    col = c(colorLikes, colorComments),
    ylim = c(0, max(likesMomentumAt(instants) + commentsMomentumAt(instants)))
)

legend(
    "topright",
    legend = c("Likes", "Comments"),
    pch = 15,
    col = c(colorLikes, colorComments),
)
```

Example showing a more realistic events scenario where an item's instant momentum calculation is affected by two indicators with different weight and dampening parameters (Likes: $l_w = `r likeWeight`$ and $l_f = `r format(likeMomentumDecayFactor, scientific = FALSE)`$, Comments: $c_w = `r commentWeight`$ and $c_f = `r format(commentMomentumDecayFactor, scientific = FALSE)`$). Each column represents a single event.
...
For a reliable implementation, instead of recalculating all ranks continuously, only ranks older than a given `rank age threshold` are updated. This ensures ranks won't ever be updated more often than specified, and will distribute compute time evenly.

- The tuning of this `rank age threshold` will determine how far from realtime the precalculated ranks are.
...
To attain a near-realtime performant and optimal rank calculation that scales better as the number of items increases, a solution is presented that recalculates rank more often for items that received a momentum-changing `event` recently.
...
```{r, echo=FALSE, fig.align="center", dpi=300}
eventAgeDays <- seq.int(0, thresholdDecayPeriodDays * 1.5)

rankAgeThreshold <- function(days) {
    rankAgeThreshold <- days * maxThresholdDays / thresholdDecayPeriodDays
    ifelse(rankAgeThreshold > maxThresholdDays, maxThresholdDays, rankAgeThreshold)
}

df <- data.frame(eventAgeDays, rankAgeThreshold(eventAgeDays))
plot(df,
    xlab = "Last event age (days)",
    ylab = "Rank age threshold",
    type = 'l'
)
```

Example result where $h_{max} = `r maxThresholdDays`$ and $t_{max} = `r thresholdDecayPeriodDays`$
